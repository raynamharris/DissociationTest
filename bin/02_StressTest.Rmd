...
dds <- DESeqDataSetFromMatrix(countData = countData,
                              colData = colData,
                              design = ~ Group + Punch + Group * Punch )
...
pcadata <- pcadataframe(rld, intgroup=c("Group", "Punch"), returnData=TRUE)
percentVar <- round(100 * attr(pcadata, "percentVar"))

## plot a bunch of pca plots using my ggplot functions DESeqPCAfunction.R, with the color defined infigureoptions.R
plotPC1PC2(aescolor = pcadata$Punch, aesshape = pcadata$Group, legendname = "Punch Group", colorvalues = colorvalpunch)
plotPC3PC4(aescolor = pcadata$Punch, aesshape = pcadata$Group, legendname = "Punch Group", colorvalues = colorvalpunch)
...
contrast4 <- resvals(contrastvector = c('Group', 'homecage', 'stressed'), mypadj = 0.1)
...
df <- as.data.frame(colData(dds)[,c("Group", "Punch")])
...
DEGes$padjmin <- with(DEGes, pmin(pvalGrouphomecagestressed)) # put the min pvalue in a new column
...
GOpvalsMDH <- GOpvals %>%
  #filter(padjGrouphomecagestressed < 0.1) %>%
  select(gene, padjGrouphomecagestressed)
GOpvalsMDH$logP <- log(GOpvalsMDH$padjGrouphomecagestressed)
