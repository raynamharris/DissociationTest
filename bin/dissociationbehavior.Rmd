---
title: "dissociationbehavior.Rmd"
...
knitr::opts_chunk$set(fig.path = '../figures/allregions_allgroups/')
```
...
colData <- colData %>% 
  filter(grepl("100|101|A|B|C|D", Mouse))  %>% 
  filter(RNAseqID != "147D-CA1-1", RNAseqID != "145B-CA3-1")  %>% ## bad samples
...
#sum(resMethodHomogDiss$padj < 0.1, na.rm = TRUE) #217
...
resYear <- results(dds, contrast = c("Year",  "Spring2016" , "Summer2015"), independentFiltering = F)
sum(resYear$padj < 0.1, na.rm = TRUE) #4
valsYear <- cbind(resYear$pvalue, resYear$padj) 
colnames(valsYear)=c("pval.Year", "padj.Year")


resPunchCA1DG <- results(dds, contrast = c("Punch", "CA1", "DG"), independentFiltering = F)
sum(resPunchCA1DG$padj < 0.1, na.rm = TRUE) # 847
...
sum(resPunchCA1CA3$padj < 0.1, na.rm = TRUE) #5
...
#head(assay(rld), 3)
rldd <- assay(rld)
rldpvals <- cbind(rldd, valsYear, valsPunchCA1DG, valsPunchCA1CA3, valsPunchCA3DG)
...
plotPCA(rld, intgroup=c("Year", "Punch"), returnData=TRUE)
pcadata <- pcaplot16(rld, intgroup=c("Year", "Punch"), returnData=TRUE)
percentVar <- round(100 * attr(pcadata, "percentVar"))

pcayear <- ggplot(pcadata, aes(PC1, PC2, color=Year, label=name)) +
  geom_point(size=5) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) +
  theme_classic() +
  stat_ellipse(level = 0.95, (aes(color=Year)),size=1)  +
  scale_color_manual(values=c("#9970ab", "#f1a340"))
...
plot2by2 <- plot_grid(pcayear,pcaregion,
                      labels=c("A", "B"), ncol = 2)
pcayear
pcaregion
plot2by2
save_plot("../figures/allregions_allgroups/plot2by2.pdf", plot2by2,
          ncol = 2, # we're saving a grid plot of 2 columns
          nrow = 1)

save_plot("../figures/allregions_allgroups/pcayear.png", pcayear)
save_plot("../figures/allregions_allgroups/pcaregion.png", pcaregion)
...
Methodyear <- row.names(rldpvals[rldpvals$padj.Year<0.1 & !is.na(rldpvals$padj.Year),])
#MethodHomogDiss <- row.names(rldpvals[rldpvals$padj.MethodHomogDiss<0.1 & !is.na(rldpvals$padj.MethodHomogDiss),])
#MethodYokedTrained <- row.names(rldpvals[rldpvals$padj.MethodYokedTrained<0.1 & !is.na(rldpvals$padj.MethodYokedTrained),])
...
## four way grid
candidates <- list("CA1 v. DG" = PunchCA1DG, "CA1 v. CA3" = PunchCA1CA3, "2015 v 2016" = Methodyear )
#dev.off()
...
candidates <- list("CA1 v. DG" = PunchCA1DG, "CA1 v. CA3" = PunchCA1CA3, "CA3 v. DG" = PunchCA3DG,  "2015 v 2016" = Methodyear )
#dev.off()
prettyvenn <- venn.diagram(lwd =3,
  scaled=T,
  x = candidates, filename=NULL, 
...
save_plot("../figures/allregions_allgroups/prettyvenn.pdf", prettyvenn,
...
df <- as.data.frame(colData(dds)[,c("Year", "Punch")])

ann_colors = list(Year = c(Summer2015 = (values=c("#f1a340")), 
                             Spring2016 = (values=c("#9970ab"))),
...
DEGes$padjmin <- with(DEGes, pmin(padj.Year, padj.CA1DG, padj.CA1CA3, padj.CA3DG)) # put the min pvalue in a new column
DEGes <- DEGes %>% filter(padjmin < 0.005)
rownames(DEGes) <- DEGes$rownames
drop.cols <- c("padj.Year" , "pval.Year" ,"padj.CA1DG" , "padj.CA1CA3" , "padj.CA3DG" , "pval.CA1DG" , "pval.CA1CA3" , "pval.CA3DG" , "rownames", "padjmin")
...
pheatmap(DEGes, show_colnames=T, show_rownames = F,
         ...
         filename = "../figures/allregions_allgroups/myheatmap.pdf"
         )
```
