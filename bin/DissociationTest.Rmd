...
colData <- colData %>% filter(grepl("Homogenized|Dissociated", Method)) %>% droplevels()
```
...
#sum(resMethodHomogDiss$padj < 0.1, na.rm = TRUE) #304 #554
...
topVarGenes <- head(order(rowVars(assay(rld)),decreasing=TRUE),75)
...
pheatmap(mat, show_colnames=F, show_rownames = TRUE,
