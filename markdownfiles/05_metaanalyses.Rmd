...
```{r VennDiagramTreatementNoDissociation, echo=FALSE, message=FALSE, results='hide',comment=FALSE, warning=FALSE}
candidates <- list("Cembrowski Region" = cembrowski_venn123, "Cognition" = cognition_venn123, "Stress" = stress_venn123)
...
# extract the intersection all experiments CA1-DG only
intersection <- Reduce(intersect, list(dissociation_venn1,stress_venn1,cognition_venn1, cembrowski_venn1))
intersection <- as.data.frame(intersection)
intersection <- t(intersection)
rownames(intersection) <- NULL
intersection <- as.data.frame(intersection)
...
write.csv(intersection, "./06_GO_MWU/05_metaanalyses_intersection_DGCA1only.csv", row.names = F)
