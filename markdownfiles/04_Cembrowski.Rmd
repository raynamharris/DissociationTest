...
contrast4 <- resvals(contrastvector = c('Location', 'dorsal', 'ventral'), mypval = 0.1)
...
DEGes$padjmin <- with(DEGes, pmin(pvalLocationdorsalventral, padjRegionCA1DG ,padjRegionCA3DG, padjRegionCA1CA3 )) # put the min pvalue in a new column
DEGes <- DEGes %>% filter(padjmin < 0.1)

#DEGes$pvalmin <- with(DEGes, pmin(pvalLocationdorsalventral, pvalRegionCA1DG ,pvalRegionCA3DG, pvalRegionCA1CA3 )) # put the min pvalue in a new column
...
```{r HeatmapPval, echo=FALSE, message=FALSE, results='hide',comment=FALSE, warning=FALSE}
## Any padj <0.1
DEGes <- assay(rld)
DEGes <- cbind(DEGes, contrast1, contrast2, contrast3, contrast4)
DEGes <- as.data.frame(DEGes) # convert matrix to dataframe
...
pheatmap(DEGes, show_colnames=F, show_rownames = F,
         ...
         cellwidth = 11, 
         #main = "Any Padj < 0.1",
         filename = "../figures/04_Cembrowski/HeatmapPval-1.pdf",
         breaks=myBreaks
         )
```
...
GOpvals <- GOpvals %>%
  select(gene, pvalLocationdorsalventral)
GOpvals$logP <- log(GOpvals$pvalLocationdorsalventral)
GOpvals <- GOpvals %>%
  select(gene, logP)
