...
knitr::opts_chunk$set(fig.path = '../figures/allregions_withbehavior/')
```
...
colData <- read.csv('../data/intermediatefiles/colData.csv')
rownames(colData) <- colData$RNAseqID
countData <-  read.csv('../data/intermediatefiles/countData.csv', check.names = F)
...
#head(assay(rld), 3)
rldd <- assay(rld)
rldpvals <- cbind(rldd, valsMethodYokedTrained, valsPunchCA1DG, valsPunchCA1CA3, valsPunchCA3DG)
...
pcaregion
plot2by2
save_plot("../figures/allregions_withbehavior/plot2by2.pdf", plot2by2,
          ncol = 2, # we're saving a grid plot of 2 columns
          nrow = 1)

save_plot("../figures/allregions_withbehavior/pcamethod.png", pcamethod)
save_plot("../figures/allregions_withbehavior/pcaregion.png", pcaregion)
...
## four way grid
candidates <- list("CA1 v. DG" = PunchCA1DG, "CA1 v. CA3" = PunchCA1CA3, "Yoked v. Trained" = MethodYokedTrained )
#dev.off()
...
candidates <- list("CA1 v. DG" = PunchCA1DG, "CA1 v. CA3" = PunchCA1CA3, "CA3 v. DG" = PunchCA3DG,  "Yoked v. Trained" = MethodYokedTrained  )
#dev.off()
prettyvenn <- venn.diagram(lwd =3,
  scaled=T,
  x = candidates, filename=NULL, 
...
save_plot("../figures/allregions_withbehavior/prettyvenn.pdf", prettyvenn,
...
DEGes$padjmin <- with(DEGes, pmin(padj.MethodYokedTrained, padj.CA1DG, padj.CA1CA3, padj.CA3DG)) # put the min pvalue in a new column
DEGes <- DEGes %>% filter(padjmin < 0.005)
rownames(DEGes) <- DEGes$rownames
drop.cols <- c("padj.MethodYokedTrained" , "pval.MethodYokedTrained" ,"padj.CA1DG" , "padj.CA1CA3" , "padj.CA3DG" , "pval.CA1DG" , "pval.CA1CA3" , "pval.CA3DG" , "rownames", "padjmin")
...
pheatmap(DEGes, show_colnames=F, show_rownames = F,
         ...
         filename = "../figures/allregions_withbehavior/myheatmap.pdf"
         )
```
