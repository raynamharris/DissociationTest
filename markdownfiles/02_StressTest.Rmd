...
summarytable$RegionTreatment <- as.factor(paste(summarytable$Region, summarytable$Treatment, sep="_"))
summarytabledf <- summarytable %>% select(RegionTreatment)  %>%  table()
summarytabledf
```
...
```{r}
res <- results(dds, contrast =c('Treatment', 'shocked', 'homecage'), independentFiltering = T, alpha = 0.1)
summary(res)
table(res$padj<0.1)
head((res[order(res$padj),]), 10)
head((res[order(res$log2FoldChange),]), 10)
...
stressresults <- data.frame(cbind("gene"=row.names(res), 
                         "baseMean" = res$baseMean,
                         "log2FoldChange" = res$log2FoldChange,
                         "lfcSE" = res$lfcSE,
                         "pvalue" = res$pvalue, "padj" = res$padj,
                         "logP"=round(-log(res$pvalue+1e-10,10),1)))
write.csv(stressresults, file = "../data/02_stress_results.csv", row.names = F)
