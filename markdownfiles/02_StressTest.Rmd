...
myhistogram(contrastvector = c('Group', 'homecage', 'stressed'), mypval = 0.1)
...
DEGes$padjmin <- with(DEGes, pmin(padjGrouphomecagestressed, padjRegionCA1DG ,padjRegionCA3DG, padjRegionCA1CA3 )) # put the min pvalue in a new column
...
DEGes$pvalmin <- with(DEGes, pmin(pvalGrouphomecagestressed, pvalRegionCA1DG ,pvalRegionCA3DG, pvalRegionCA1CA3 )) # put the min pvalue in a new column
DEGes <- DEGes %>% filter(pvalmin < 0.1)
...
GOpvals <- GOpvals %>%
  select(gene, padjGrouphomecagestressed)
GOpvals$logP <- log(GOpvals$padjGrouphomecagestressed)
GOpvals <- GOpvals %>%
  select(gene, logP)

write.csv(GOpvals, "./06_GO_MWU/02_stress_GOpvals.csv", row.names = F)
```