...
myhistogram(contrastvector = c('Treatment', 'dissociated', 'homogenized'), mypval = 0.1)

```

This Venn Diagram sthe overlap of differentailly expression genes by Region and Treatment. This shows all genes with *uncorrected* pvalue <0.1.
...
DEGes$padjmin <- with(DEGes, pmin(padjTreatmentdissociatedhomogenized, padjRegionCA1DG ,padjRegionCA3DG, padjRegionCA1CA3 )) # put the min pvalue in a new column
DEGes <- DEGes %>% filter(padjmin < 0.1)

#DEGes$pvalmin <- with(DEGes, pmin(pvalTreatmentdissociatedhomogenized, pvalRegionCA1DG ,pvalRegionCA3DG, pvalRegionCA1CA3 )) # put the min pvalue in a new column
...
#head(assay(rld))
colnames(rld) <- paste(colData$Region, colData$Treatment, colData$RNAseqID, sep = "")
#head(assay(rld))
...
resCD=results(dds, contrast=c('Treatment', 'dissociated', 'homogenized'))
