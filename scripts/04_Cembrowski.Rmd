...
colData$Subfield <- plyr::revalue(colData$Subfield, c("ca2"="CA2"))
colData$Subfield <- plyr::revalue(colData$Subfield, c("ca4"="CA4"))
...
dim( countData )
colSums( countData ) / 1e06  # in millions of reads
table( rowSums( countData ) )[ 1:30 ] # Number of genes with low counts
...
```{r viewdata, echo=FALSE,message=FALSE, warning=FALSE}
...
write.csv(pcadata, "../results/04_cembrowksi_pcadata.csv")
write.csv(percentVar, "../results/04_cembrowksi_percentVar.csv")
```

```{r PCAstats, echo=TRUE, message=FALSE, comment=FALSE, warning=FALSE}
summary(aov(PC1 ~ Subfield, data=pcadata)) 
TukeyHSD(aov(PC1 ~ Subfield, data=pcadata), which = "Subfield") 

summary(aov(PC2 ~ Subfield, data=pcadata)) 
...
The bottom heat map is a much less stringent cutoff and this one cleanly separates first by brain Subfield and then by dorsal ventral Treatment.
...
GOpvals <- GOpvals %>%
  select(gene, pvalTreatmentventraldorsal)
