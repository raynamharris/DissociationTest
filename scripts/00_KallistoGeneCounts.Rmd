...
These files and this analysis will take up considerable space and time. If you want to run the analysis, first download `GSE99765_Dissociation` from this GitHub repository: https://github.com/raynamharris/MouseHippocampusRNAseqData and save them in this repo in `../data/. 
...
# tpm to prkcz_tpm - used just for pkmz
prkcz_tpm <- full_join(geneids, tpm) # merge tpm and genids
prkcz_tpm <- prkcz_tpm[-c(1,2,4,5,12)]   ## keep gene name and tpm for samples)
prkcz_tpm <- dplyr::filter(prkcz_tpm, grepl('Prkcz', gene)) %>%
                         arrange(transcript)
prkcz_tpm
prkcz_tpm <- prkcz_tpm[-c(1,3:7)]   ## keep gene name and tpm for samples)
row.names(prkcz_tpm) <- prkcz_tpm$transcript ## make gene the row name
prkcz_tpm[1] <- NULL ## make gene the row name
prkcz_tpm <- round(prkcz_tpm) #round all value to nearest 1s place
...
write.csv(geneids, "../data/geneids.csv", row.names=F)
write.csv(tpmbytranscript, "../data/GSE99765_Dissociation_tpmbytranscript.csv", row.names=T)
write.csv(prkcz_tpm, "../data/GSE99765_Dissociation_prkcz_tpm.csv", row.names=T)
