...
gomwuPlot=function(inFile,goAnnotations,goDivision,level1=0.1,level2=0.05,level3=0.01,absValue=-log(0.05,10),adjusted=TRUE,txtsize=1,font.family="sans",treeHeight=0.5,colors=NULL) {
  ...
  for (i in length(labs):1) {
    ...
    if (goods$pval[i]> -log(level3,10)) { 
      text(left,ypos,labs[i],font=1,cex=1*txtsize,col=goods$color[i],adj=c(0,0),family=font.family) 
    } else {
  ...
  # add group colom
  goods$enriched <- ifelse(grepl("dodgerblue2", goods$color), "HOMO", 
                                         ifelse(grepl("firebrick1", goods$color), "DISS", NA))
  
  # save file for manuscipt 
  write.csv(goods, paste0("goods", goDivision, level1, level3, ".csv"))
