...
```{r heatmap}
contrast4 <- resvals(contrastvector = c('Treatment', 'ventral', 'dorsal'), mypval = 0.01)
...
DEGes$padjmin <- with(DEGes, pmin(padjTreatmentventraldorsal)) # put the min pvalue in a new column

write.csv(as.data.frame(DEGes), "../results/04_cembrowksi_DEGes.csv", row.names = F)
```
...
summary(data)
write.csv(data, "../results/04_cembrowski_volcanoTreatment.csv")
...
data$color <- as.factor(data$color)
summary(data)
write.csv(data, "../results/04_cembrowski_volcanoCA1DG.csv")
```
